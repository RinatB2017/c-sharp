...
namespace UsbLibrary
{
    ...
    public abstract class HIDDevice : Win32Usb, IDisposable
    {
        ...
        /// <summary>Длина Feature Report : берём из девайса</summary>
        private int m_nFeatureReportLength;
            ...
            if ( m_hHandle != InvalidHandleValue || m_hHandle == null)	// if the open worked...
			{
				...
				if (HidD_GetPreparsedData(m_hHandle, out lpData))	// get windows to read the device data into an internal buffer
				{
                    try
                    {
                        ...
                        m_nFeatureReportLength = oCaps.FeatureReportByteLength; // ну и конечно же дописываем Feature Reports длину
        ...
        protected bool ReadManufacturerString(ref byte[] arrBuff)
        {
            //byte[] arrBuff = new byte[m_nFeatureReportLength];
            return HidD_GetManufacturerString(m_hHandle,arrBuff, arrBuff.Length);
        }

        protected bool ReadProductString(ref byte[] arrBuff)
        {
            return HidD_GetProductString(m_hHandle, arrBuff, arrBuff.Length);
        }
		...
		private static string GetDevicePath(IntPtr hInfoSet, ref DeviceInterfaceData oInterface)
		{
			...
			if (!SetupDiGetDeviceInterfaceDetail(hInfoSet, ref oInterface, IntPtr.Zero, 0, ref nRequiredSize, IntPtr.Zero))
			{
                ...
                // hardcoded to 5! 
                // Sorry, but this works and trying more future proof versions by setting the size to the struct sizeof failed miserably. 
                // If you manage to sort it, mail me! Thx
                oDetail.Size = 5;	
        ...
        public virtual FeatureReport CreateFeatureInReport()
        {
            return null;
        }
		#endregion
